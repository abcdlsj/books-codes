            ...
            char *status_msg) /* Status message returned to caller */
{
    ...
    {
        strcpy(status_msg, "Error: Result string contains an illegal or "
    ...
    {
        strcpy(status_msg,
               "Error: Client unable to write to the Autolab server");
        close(clientfd);
        return -1;
    }
...
int driver_post(char *userid, char *result, int autograded, char *status_msg)
{
    ...
    {
        status = submitr(SERVER_NAME, SERVER_PORT, COURSE_NAME, userid, LAB,
                         result, status_msg);
        return status;
    }
