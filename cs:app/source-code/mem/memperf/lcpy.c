...
double getcpufrequency()
{
    ...
    /* if used on a MP system, and hw counters are not (almost) synchronized,
       bad things may happen! on some unix systems it would be possible to use
...
DWORD WINAPI memop(paramT *p)
{
    ...
    p->mxiters, p->par_cid); fflush(stdout);
    */
    ...
    for (li = 0, y = 1; (strdr[li] != -1) && (li < p->mxstrds); li++, y++)
    {
        ...
        DEBUGOUT
        {
            if (p->par_cid == 0)
            {
                ...
                fprintf(stream, "%4s %6s %6s %16s %16s %16s %10s\n", "proc",
                        "stride", "iters", "blk size", "eff. size", "exec.time",
                        "bandwidth");

                fprintf(stream, "%4s %6s %6s %16s %16s %16s %10s\n", "#",
                        "bytes", "#", "bytes", "bytes", "clocks", "MB/sec");
            }
        }
        ...
        for (x = 1, mx = p->minsize; mx <= p->mxsize; mx *= 2, x++)
        {
            ...
            case 2:
                DEBUGOUT printf(
                    "mx %d: t1 %d, t2 %d, t4 %d, t8 %d, topt %d  \n ", mx, t1,
                    t2, t4, t8, topt);
                t1 = t1 < t2 ? t1 : t2;
                t1 = t1 < t4 ? t1 : t4;
                t1 = t1 < t8 ? t1 : t8;
...
void analyze_rep(paramT *p)
{
    ...
    for (itloop = 0; itloop < nrofrep; itloop++)
    {
        for (ploop = 0; ploop < p->npes; ploop++)
        {
            ...
            strcat(namebw, number);
            strcat(namebw, "f");
            /* we need the "b" option in fopen for windows only, hopefully all
             * other systems will ignore it */
    ...
    if (nrofrep * p->npes > 1)
    {
        ...
        printf("          arithmetic mean of all (percentage) standard "
               "deviations: %4.2f %%\n",
        ...
        printf("          warning: no repetitions, results likely contain "
               "badly wrong values\n");
};
...
int __cdecl main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        printf("Usage: %s -m <mode> [-p] [-s] [-n] [-r] [-i] [-t] [-c] [-a] "
               "[-o]\n",
               argv[0]);
#else
...
#ifdef HAVEOPT
        printf("       [-a <optasm> ]   0/1 = use/don't use special "
               "instructions\n");
    ...
    for (modenr = 0; modenr <= 3; modenr++)
    {
        if ((modenr == modes) || (modes == 9))
        {
            ...
            for (currit = 1; currit <= nrofrep; currit++)
            {
                ...
                {
                case 0:
                    printf("Load Sum (%d)    - Working Set: %.0f KByte; "
                           "Strides: %d\n",
                           currit, (float)p[0].mxsize * 8 / 1024, p[0].mxstrds);
                    break;
                case 1:
                    printf("Const Store (%d) - Working Set: %.0f KByte; "
                           "Strides: %d\n",
                           currit, (float)p[0].mxsize * 8 / 1024, p[0].mxstrds);
                    break;
                case 2:
                    printf("Load Copy (%d)   - Working Set: 2*%.0f KByte; "
                           "Strides: %d\n",
                           currit, (float)p[0].mxsize * 8 / 1024, p[0].mxstrds);
                    break;
                case 3:
                    printf("Copy Store (%d)  - Working Set: 2*%.0f KByte; "
                           "Strides: %d\n",
                           currit, (float)p[0].mxsize * 8 / 1024, p[0].mxstrds);
                    break;
                }
                ...
                for (i = 0; i < p[0].npes; i++)
                {
                    ...
                    */
                    hThread[i] =
                        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)memop,
                                     &p[i], 0, &IDThread);
